...
from calculator import VedicAstroCalculator  # noqa: E402
...
class TestVedicAstroCalculator(unittest.TestCase):
    ...
    def test_calculate_raw_positions(self) -> None:
        ...
        Validates output bounds, types and node relations.
        """
    ...
    def test_solve_kepler_validation(self) -> None:
        ...
        with self.assertRaises(TypeError):
            self.calc.solve_kepler("M", 0.1)  # type: ignore
        with self.assertRaises(TypeError):
            self.calc.solve_kepler(100, "e")  # type: ignore
    ...
    def test_calculate_heliocentric_validation(self) -> None:
        ...
        with self.assertRaises(TypeError):
            self.calc.calculate_heliocentric(123, self.jd)  # type: ignore
        with self.assertRaises(TypeError):
            self.calc.calculate_heliocentric("Earth", "d")  # type: ignore
    ...
    def test_jd_validation(self) -> None:
        """
        Test input validation for methods taking Julian Date.
        Validates appropriate TypeErrors are raised for invalid inputs.
        """
        # Test methods taking jd/d raise TypeError for non-numbers
        with self.assertRaises(TypeError):
            self.calc.calculate_moon_geocentric("d")  # type: ignore
        with self.assertRaises(TypeError):
            self.calc.get_lahiri_ayanamsa("jd")  # type: ignore
        with self.assertRaises(TypeError):
            self.calc.calculate_raw_positions("jd")  # type: ignore
        with self.assertRaises(TypeError):
            self.calc.calculate_nirayana_longitudes("jd")  # type: ignore
